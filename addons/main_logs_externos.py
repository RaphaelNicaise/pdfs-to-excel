...
def abrir_popup_logs(archivos, excel_path):
    ...
    def run():
        try:
            print("Arranca el procesamiento")
            main_process(archivos, excel_path)
            print(f"\n Archivos convertidos correctamente en: {excel_path}")
            ...
            popup.bind("<Return>", cerrar_popup)
            popup.protocol("WM_DELETE_WINDOW", cerrar_popup)
...
def convertir_a_excel():
    ...
    mostrar_logs = convertir_a_excel.mostrar_logs.get()
    ...
    def ejecutar():
        try:
            if mostrar_logs:
                abrir_popup_logs(archivos, excel_path)
            else:
                main_process(archivos, excel_path)
...
def accion_seleccionar_carpeta(app, frame, etiqueta, boton_convertir, contador_pdfs):
    ...
    if carpeta:
        ...
        etiqueta.configure(
            text=f"📁 {carpeta}", anchor="w", cursor="hand2", fg_color="#2e3b4e", corner_radius=8, text_color="white")
...
def main():
    ...
    var_logs = ctk.BooleanVar(value=False)
    checkbox_logs = ctk.CTkCheckBox(
        columna_derecha,
        text="Mostrar logs en ventana emergente",
        variable=var_logs
    )
    checkbox_logs.pack(pady=(10, 0))
    convertir_a_excel.mostrar_logs = var_logs
